...
# call from one part of the program to another. However, consider this:
# now you can override the factory method in a subclass and change the
# class of products being created by the method.
...
# The code that uses the factory method (often called the client code) doesn't see
# a difference between the actual products returned by various subclasses.
...
# uses the product.
# Therefore it's easier to extend the product construction code independently from
# the rest of the code.
# For example, to add a new product type to the app, you'll only need to create
# a new creator subclass
...
# But how would the framework recognize that your subclass should be
# used instead of a standard component?

# The solution is to reduce the code that constructs components across the framework
# into a single factory method and let anyone override this method in
...
# But now you need to tell
# the main UIFramework class to use the new button subclass instead of a default one.

# To achieve this, you create a subclass UIWithRoundButtons from a base framework class and override
# its createButton method. While this method returns Button objects in the base class, you make your
# subclass return RoundButton objects. Now use the UIWithRoundButtons class instead of UIFramework.
# And that's about it!
...
#   a. First, you need to create some storage to keep track of all of the created objects.
#   b. When someone requests an object, the program should look for a free object inside that pool.
#   c. … and then return it to the client code.
#   d. If there are no free objects, the program should create a new one (and add it to the pool).
...
# placed is the constructor of the class whose objects
# we're trying to reuse. However, a constructor must always return new objects by definition.
# It can't return existing instances.

# Therefore, you need to have a regular method capable of creating new objects
# as well as reusing existing ones.
# That sounds very much like a factory method.
...
# 4. You might need to add a temporary parameter to the
# factory method to control the type of returned product.

# 5. At this point, the code of the factory method may look pretty ugly.
# It may have a large switch statement that picks which product class to instantiate.
# But don't worry, we'll fix it soon enough.

# 6. Now, create a set of creator subclasses for each type of product listed in the factory method.
# Override the factory method in the subclasses and extract the appropriate
...
# 8. For instance, imagine that you have the following hierarchy of classes:
#     the base Mail class with a couple of subclasses: AirMail and GroundMail;
#     the Transport classes are Plane, Truck and Train.
#       While the AirMail class only uses Plane objects,
#     GroundMail may work with both Truck and Train objects.
#       You can create a new subclass (say TrainMail)
#     to handle both cases, but there's another option.
#     The client code can pass an argument to the factory method of the GroundMail
#       class to control which product it wants to receive.

# 9. If, after all of the extractions, the base factory method has become empty,
#    you can make it abstract. If there's something left,
#    you can make it a default behavior of the method.
...
# You can use Factory Method along with Iterator to let collection subclasses return different
# types of iterators that are compatible with the collections.
...
class Transport(ABC):
    """
    Provides an interface for the various modes of transporations.
    """

    @abstractmethod
    def deliver(self) -> str:
        """
        Describes the mode of delivery
        """
...
class Truck(Transport):
    """
    Generates a truck delivery assignment. Inherits from Transport
    """
...
class Ship(Transport):
    """
    Generates a ship delivery assignment. Inherits from Transport
    """
...
class Logistics(ABC):
    """
    Generates the necessary logistics to deliver items.
    """
    def plan_delivery(self) -> Transport:
        """
        Plans a delivery by issueing the correct vehicle.
        """
    ...
    def create_transport(self):
        """
        Provides an adequate vehicle for delivery
        """
        raise NotImplementedError
...
class RoadLogistics(Logistics):
    """
    Generates the necessary requirement for land delivery.
    """
    def create_transport(self):
        return Truck()
...
class SeaLogistics(Logistics):
    """
    Generates the necessary requirement for sea delivery.
    """
    def create_transport(self):
        return Ship()
